...
export class LevelController {
    ...
    private amountOffScreen: number
    private time: number
    ...
    start(onPassed?: () => void, onFailed?: () => void): void {
    ...
    update(): void {
        ...
        this.amountOffScreen += oofScreenDots.length

        const passed: boolean = this.passed
        if (!passed && !this.stopped) {
            if (this.currentGroup.amount === this.amountOffScreen) {
                this.groupIndex += 1
                this.amount = this.currentGroup.amount
                this.amountOffScreen = 0
                this.leftColor = this.currentGroup.leftColor
            ...
            if (this.settings.cycle) {
                this.start(this.onPassed, this.onFailed)
            } else {
                this.stopped = true
    ...
    private get balance(): number {
        return Number(Designer.balanceText.text)
    }
    ...
    private startDot(): void {
        for (const zone of this.currentGroup.zones.slice(0, this.amount)) {
            ...
            const x: number = MathUtils.randomBetween(zone.start, zone.end || zone.start)
